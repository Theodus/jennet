...
interface val Logger
  fun apply(ip: String val, request: http.Payload val,
    response: http.Payload val): Any tag
  fun out(ip: String val, message: String val): Any tag
...
class CommonLog is Logger
  ...
  fun val apply(ip: String val, request: http.Payload val,
    response: http.Payload val)
  =>
  ...
  fun out(ip: String val, msg: String val) =>
    let list = recover Array[String](24) end
